...
researcher = Agent(
    ...
    llm = ChatOpenAI(model_name = gpt3, temperature = 0.7)
)
...
task_google = Task(
    ...
    Find the fisrt 10 results on google, select the most interesting. 
    """,
    expected_output= "A link to the most interesting topic about LLMs. Deliver it with link to the source",
    agent = researcher
)
...
task_reddit = Task(
    ...
    """,
    expected_output= "A full comprhensive summary in detailed and long format for the selected most interesting topic found. Include link to the source.",
...
task_drafting = Task(
    description = """
    take the link as imput, go read it and summarize it. Make sure that in the final summary the source (name and link) is also mentioned. 
    """,
    expected_output= "A summary of the web page that arrived to you. Make sure the sources for each topic (link, urls) are mentioned, it is very important.",
    agent = editor
)
...
crew = Crew(
    agents = [researcher, scraper, editor],
    tasks = [task_google, task_reddit, task_drafting],
    verbose = 2,
    manager_llm=ChatOpenAI(temperature=0.6, model="gpt-4"),  # Mandatory for hierarchical process
